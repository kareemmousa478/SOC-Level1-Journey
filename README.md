# 🛡️ SOC Level 1 Journey

Welcome to my cybersecurity learning portfolio.

This repository documents my progress through the TryHackMe SOC Level 1 path, including notes, incident reports, and practical investigations.

---

## 🎯 Objectives

- Build practical SOC Analyst skills
- Learn Security Operations Center workflows
- Practice alert triage and incident investigation
- Develop hands-on blue team experience
- Create a professional cybersecurity portfolio

---

## 📚 Progress

- [x] Section 1 – Blue Team Introduction
- [x] Section 2 – SOC Team Internals
- [ ] Section 3 – Core SOC Solutions
- [ ] Section 4 – Cyber Defence Frameworks
- [ ] Section 5 – Phishing Analysis
- [ ] Section 6 – Network Traffic Analysis
- [ ] Section 7 – Network Security Monitoring
- [ ] Section 8 – Web Security Monitoring
- [ ] Section 9 – Windows Security Monitoring
- [ ] Section 10 – Linux Security Monitoring
- [ ] Section 11 – Malware Concepts
- [ ] Section 12 – Threat Analysis Tools
- [ ] Section 13 – SIEM Triage for SOC
- [ ] Section 14 – SOC Level 1 Capstone

---

## 🛠️ Skills

- Blue Team Operations
- SOC Fundamentals
- Alert Triage
- Incident Reporting
- Security Monitoring
- SIEM
- Threat Detection

---

## 🚀 Status

Currently studying the TryHackMe SOC Level 1 learning path and documenting everything learned.
